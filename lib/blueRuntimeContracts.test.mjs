...
import { modelsForAccess, normalizeOpenRouterModels, providerModelId, publicModel, resolveModel } from './openrouter.ts';
import {
...
test('catalogue aliases resolve to observed permanent model routes', () => {
...
test('dated canonical billing IDs retain the live free route for guardrail provisioning', async () => {
  const models = normalizeOpenRouterModels([{
  ...
  const model = resolveModel(models, 'cohere/north-mini-code-20260617');
  assert.equal(model.id, 'cohere/north-mini-code-20260617:free');
  assert.equal(providerModelId(model), 'cohere/north-mini-code:free');
  assert.equal(publicModel(model).id, 'cohere/north-mini-code:free');
  assert.equal(resolveModel(models, 'cohere/north-mini-code:free'), model);
  const previousKey = process.env.OPENROUTER_MANAGEMENT_API_KEY;
  const previousWorkspace = process.env.OPENROUTER_WORKSPACE_ID;
  const previousFetch = globalThis.fetch;
  ...
  globalThis.fetch = async (_url, init = {}) => {
    ...
    assert.deepEqual(body.allowed_models, ['cohere/north-mini-code:free']);
    return Response.json({ data: { id: 'cohere-free-guardrail' } });
  };
  try {
    assert.equal(await createModelGuardrail(providerModelId(model)), 'cohere-free-guardrail');
  } finally {
    globalThis.fetch = previousFetch;
    if (previousKey === undefined) delete process.env.OPENROUTER_MANAGEMENT_API_KEY;
    else process.env.OPENROUTER_MANAGEMENT_API_KEY = previousKey;
...
test('paid and free routes keep separate identities, pricing and access in either catalogue order', () => {
  const rows = [{
    id: 'vendor/model', canonical_slug: 'vendor/model-20260907',
    pricing: { prompt: '0.000001', completion: '0.000002' }, supported_parameters: ['tools']
  }, {
    id: 'vendor/model:free', canonical_slug: 'vendor/model-20260907',
    pricing: { prompt: '0', completion: '0' }, supported_parameters: ['tools']
  }, {
    id: 'vendor/model:batch', canonical_slug: 'vendor/model-20260907',
    pricing: { prompt: '0.0000005', completion: '0.000001' }, supported_parameters: ['tools']
  }];
  for (const catalogue of [rows, [...rows].reverse()]) {
    const models = normalizeOpenRouterModels(catalogue);
    assert.equal(models.length, 2);
    const paid = resolveModel(models, 'vendor/model');
    const free = resolveModel(models, 'vendor/model:free');
    assert.notEqual(paid, free);
    assert.equal(paid.id, 'vendor/model-20260907');
    assert.equal(free.id, 'vendor/model-20260907:free');
    assert.equal(publicModel(paid).isFree, false);
    assert.equal(publicModel(free).isFree, true);
    assert.equal(publicModel(paid).upstreamModel, 'vendor/model');
    assert.equal(publicModel(free).upstreamModel, 'vendor/model:free');
    assert.deepEqual(modelsForAccess(models, 'trial'), [free]);
    assert.equal(resolveModel(models, 'vendor/model-20260907'), undefined,
      'an ambiguous old canonical selection must not silently become paid');
    assert.equal(resolveModel(models, 'vendor/model:batch'), undefined);
    assert.equal(resolveModel(models, 'vendor/model-20260907:batch'), undefined);
    assert.equal(resolveModel(models, 'vendor/model-20260907:free'), free);
    assert.equal(resolveModel(models, 'vendor/model-20260907', false), paid,
      'a persisted paid task must retain its reserved pricing class');
    assert.equal(resolveModel(models, 'vendor/model-20260907', true), free,
      'an older persisted free task must never switch to the paid row');
    assert.equal(resolveModel(models, 'vendor/model', true), undefined);
    assert.equal(resolveModel(models, 'vendor/model:free', false), undefined);
  }
});

test('catalogue never invents a dated route for a moving-only alias or malformed row', () => {
  const models = normalizeOpenRouterModels([null, {}, {
    id: '~vendor/only-alias', canonical_slug: 'vendor/model-20260907',
    pricing: { prompt: '0', completion: '0' }, supported_parameters: ['tools']
  }, {
    id: 'invalid route', canonical_slug: 'vendor/model', supported_parameters: ['tools']
  }, {
    id: 'vendor/wrong-tools', supported_parameters: 'tools'
  }]);
  assert.deepEqual(models, []);
  assert.equal(providerModelId({ id: '~vendor/only-alias', canonical_slug: 'vendor/model-20260907' }), undefined);
  assert.equal(providerModelId({ id: 'vendor/model', provider_route_id: 'invalid route' }), undefined);
});

test('variant aliases cannot cross a paid/free boundary or take another model canonical ID', () => {
  const models = normalizeOpenRouterModels([{
    id: 'vendor/public', canonical_slug: 'vendor/model-20260907',
    aliases: ['vendor/public:free', 'vendor/public:batch', 'vendor/other-20260907'],
    supported_parameters: ['tools'], pricing: { prompt: '1', completion: '1' }
  }, {
    id: 'vendor/other', canonical_slug: 'vendor/other-20260907',
    supported_parameters: ['tools'], pricing: { prompt: '1', completion: '1' }
  }]);
  assert.equal(resolveModel(models, 'vendor/public:free'), undefined);
  assert.equal(resolveModel(models, 'vendor/public:batch'), undefined);
  assert.equal(resolveModel(models, 'vendor/other-20260907').provider_route_id, 'vendor/other');
});

test('legacy proxy keeps canonical billing separate from the observed provider route', async () => {
  const route = await readFile(new URL('../app/api/chat/completions/route.ts', import.meta.url), 'utf8');
  assert.match(route, /reserveUsage\(account, requestId, model\.id, reservationAmount\)/);
  assert.match(route, /const upstreamPayload = \{\s*\.\.\.body,\s*model: providerModelId\(model\)/);
  assert.match(route, /ENABLE_LEGACY_AI_PROXY/);
});
...
test('child-key verification rejects additional route variants sharing one canonical ID', async () => {
  const previousFetch = globalThis.fetch;
  globalThis.fetch = async () => Response.json({ data: [
    { id: 'vendor/model', canonical_slug: 'vendor/model-20260907' },
    { id: 'vendor/model:free', canonical_slug: 'vendor/model-20260907' }
  ...
  try {
    await assert.rejects(
      assertManagedKeyCanUseModel('child-key', 'vendor/model:free', ['vendor/model-20260907']),
      /not restricted to the selected model/
    );
  } finally {
    globalThis.fetch = previousFetch;
...
test('runtime verifies exact child-key model access after assigning its guardrail', async () => {
  ...
  const verification = runtime.indexOf('await assertManagedKeyCanUseModel(created.key, providerRoute, [providerModel.id])');
  assert.match(runtime, /await ensureModelGuardrail\(providerRoute\)/);
  assert.match(runtime, /credential\.model = providerModelId\(model\)/);
...
test('failed runtime admissions and settlements expose a redacted terminal reason', async () => {
  const runtime = await readFile(new URL('./blueRuntime.ts', import.meta.url), 'utf8');
  const admission = runtime.slice(runtime.indexOf('function admissionPayload('), runtime.indexOf('function existingSettlement('));
  const settlement = runtime.slice(runtime.indexOf('function existingSettlement('), runtime.indexOf('async function loadBillingAccount('));
  assert.match(admission, /\.\.\.publicTerminalReason\(task\)/);
  assert.match(settlement, /\.\.\.publicTerminalReason\(task\)/);
  const reason = runtime.slice(runtime.indexOf('function publicTerminalReason('));
  assert.match(reason, /task\.state !== 'failed'/);
