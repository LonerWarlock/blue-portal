...
test('Meta Pixel is installed globally and tracks client-side page views', async () => {
  const [layout, metaPixel] = await Promise.all([
    readFile(new URL('../app/layout.tsx', import.meta.url), 'utf8'),
    readFile(new URL('../app/components/MetaPixel.tsx', import.meta.url), 'utf8')
  ]);

  assert.match(layout, /1104103875476885/);
  ...
  assert.match(layout, /facebook\.com\/tr\?id=\$\{META_PIXEL_ID\}&ev=PageView&noscript=1/);
  assert.match(layout, /<MetaPixelPageView \/>/);
  assert.match(metaPixel, /window\.fbq\?\.\("track", "PageView"\)/);
  assert.match(metaPixel, /\[pathname\]/);
});
