...
const inputClass = 'w-full px-4 py-2.5 rounded-lg bg-paper-alt border border-line text-ink text-sm focus:outline-none focus:border-brand focus:ring-1 focus:ring-brand/20 transition placeholder:text-ink-faint';
    ...
    return (
      <div className="min-h-screen bg-paper flex items-center justify-center px-6">
        <div className="absolute inset-0 pointer-events-none">
          <div className="absolute top-1/4 left-1/4 w-96 h-96 bg-brand/10 rounded-full blur-[128px]" />
          <div className="absolute bottom-1/4 right-1/4 w-96 h-96 bg-brand/10 rounded-full blur-[128px]" />
        </div>
        <div className="rounded-lg panel border border-line p-10 max-w-lg w-full text-center relative z-10">
          {paymentResult === 'success' ? (
            <>
              ...
              <h1 className="text-2xl font-bold text-ink mb-3">Registration Complete!</h1>
              <p className="text-ink-muted text-sm mb-6">
                Your internship registration with <span className="text-brand font-semibold">Imergene</span> has been confirmed.
                You will receive further details at your registered email address.
              </p>
              <div className="rounded-lg bg-paper-alt border border-line p-4 text-left text-xs text-ink-muted space-y-1 mb-6">
                {resultData && (
                  <>
                    <p><span className="text-ink-faint">Name:</span> <span className="text-ink">{resultData.name}</span></p>
                    <p><span className="text-ink-faint">Email:</span> <span className="text-ink">{resultData.email}</span></p>
                  </>
                )}
                <p><span className="text-ink-faint">Amount Paid:</span> <span className="text-green-400 font-semibold">{INTERNSHIP_FEE_STR}</span></p>
              </div>
              <a href="/internships" className="inline-flex items-center gap-2 px-6 py-3 rounded-lg bg-brand font-semibold text-white text-sm shadow-lg transition">
            ...
            <>
              ...
              <h1 className="text-2xl font-bold text-ink mb-3">Payment Failed</h1>
              <p className="text-ink-muted text-sm mb-6">
              ...
              <div className="flex gap-3 justify-center">
                <button onClick={() => { setStep(4); setForm(f => ({ ...f, declarationAccepted: false, termsAccepted: false })); window.history.replaceState({}, '', '/internships'); }}
                  className="px-6 py-3 rounded-lg bg-brand font-semibold text-white text-sm shadow-lg transition">
                  Try Again
                </button>
                <a href="/" className="px-6 py-3 rounded-lg border border-line text-ink-muted text-sm hover:text-ink hover:border-line-strong transition">
                  Go Home
                </a>
              </div>
            </>
    ...
    switch (step) {
      case 0: return (
        <div className="space-y-4">
          ...
          <p className="text-xs text-ink-faint mb-4">Tell us about yourself.</p>
          ...
          <div>
            ...
            <p className="text-[12px] text-ink-muted mt-1">Please provide correct email address. Form confirmation details will be sent here.</p>
      ...
      case 1: return (
        <div className="space-y-4">
          <h2 className="text-lg font-bold text-ink mb-1">Address Details</h2>
          <p className="text-xs text-ink-faint mb-4">Your current and permanent address.</p>

          <div className="rounded-lg bg-paper-alt border border-line p-4">
            <h3 className="text-sm font-semibold text-ink-muted mb-3">Current Address</h3>
          ...
          <label className="flex items-center gap-2 cursor-pointer select-none">
            <input type="checkbox" checked={copyPermAddr} onChange={e => setCopyPermAddr(e.target.checked)}
              className="w-4 h-4 rounded border-line-strong bg-paper-alt text-brand focus:ring-brand/20" />
            <span className="text-xs text-ink-muted">Permanent address is the same as current</span>
          </label>

          <div className="rounded-lg bg-paper-alt border border-line p-4">
            <h3 className="text-sm font-semibold text-ink-muted mb-3">Permanent Address</h3>
            <div className="space-y-3">
              <div>
                <label className={labelClass}>Street Address {reqMark}</label>
      ...
      case 2: return (
        <div className="space-y-4">
          <h2 className="text-lg font-bold text-ink mb-1">UG Academic Details</h2>
          <p className="text-xs text-ink-faint mb-4">Your undergraduate education information.</p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
          ...
          <div>
            ...
            <p className="text-[12px] text-ink-muted mt-1">Please write the complete college name including city</p>
          ...
          <div>
            ...
            <p className="text-[12px] text-ink-muted mt-1">Please write the complete university name including city</p>
      ...
      case 3: return (
        <div className="space-y-4">
          <h2 className="text-lg font-bold text-ink mb-1">Skills & Experience</h2>
          <p className="text-xs text-ink-faint mb-4">Tell us about your technical skills and background.</p>
          <div>
            <label className={labelClass}>Technical Skills {reqMark} <span className="text-ink-muted font-normal">(comma-separated)</span></label>
          ...
          <div>
            <label className={labelClass}>Programming Languages <span className="text-ink-muted font-normal">(comma-separated)</span></label>
            ...
            <p className="text-[12px] text-ink-muted mt-1">List programming languages you know</p>
          </div>
          <div>
      ...
      case 4: return (
        <div className="space-y-5">
          <h2 className="text-lg font-bold text-ink mb-1">Declaration & Payment</h2>
          ...
          <div className="rounded-lg bg-paper-alt border border-line overflow-hidden">
            <div className="sticky top-0 z-10 bg-paper-alt border-b border-line px-4 py-2.5">
              <h3 className="text-sm font-bold text-ink">Imergene — Internship Registration Terms & Conditions</h3>
            </div>
            <div className="max-h-48 overflow-y-auto px-4 py-3 space-y-2">
              <ol className="text-xs text-ink-muted list-decimal list-inside space-y-1.5">
          ...
          <div className="space-y-3">
            <label className="flex items-start gap-3 cursor-pointer select-none group">
              <input type="checkbox" checked={form.declarationAccepted} onChange={e => set('declarationAccepted', e.target.checked)}
                className="w-4 h-4 mt-0.5 rounded border-line-strong bg-paper-alt text-brand focus:ring-brand/20 shrink-0" />
              <span className="text-xs text-ink-muted group-hover:text-ink-muted transition">
    ...
    <div className="min-h-screen bg-paper flex items-center justify-center px-4 py-10">
      ...
      <div className="w-full max-w-2xl relative z-10">
        ...
        <div className="text-center mb-8">
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-md border border-line-strong bg-paper eyebrow mb-4">
        ...
        {/* Form Card */}
        <div className="rounded-lg panel border border-line p-6 sm:p-8">
          {renderStep()}

          {error && (
            <div className="mt-4 p-3 rounded-lg bg-red-900/20 border border-red-800/50 text-red-400 text-xs flex items-start gap-2">
          ...
          <div className="flex items-center justify-between mt-6 pt-5 border-t border-line">
            {step > 0 ? (
              <button onClick={prevStep} className="px-5 py-2.5 rounded-lg border border-line text-ink-muted text-sm font-medium hover:text-ink hover:border-line-strong transition flex items-center gap-2">
                <i className="fa-solid fa-arrow-left text-xs"></i> Back
              </button>
            ) : <div></div>}

            {step < 4 ? (
              <button onClick={nextStep} className="px-6 py-2.5 rounded-lg bg-brand font-semibold text-white text-sm shadow-lg transition flex items-center gap-2">
                Next <i className="fa-solid fa-arrow-right text-xs"></i>
              </button>
            ) : (
              <button onClick={handlePayment} disabled={loading}
                className="px-6 py-2.5 rounded-lg bg-brand font-semibold text-white text-sm shadow-lg transition flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed">
                {loading ? (
                  <>
