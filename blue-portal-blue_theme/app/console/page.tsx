...
const TIER_BADGE_COLORS: Record<string, string> = {
  ...
  "Standard": "bg-brand/10 text-brand border border-brand/30",
  "High Cost": "bg-yellow-950/60 text-yellow-400 border border-yellow-900/60",
  "Premium": "bg-brand/10 text-brand border border-brand/30",
};
  ...
  return (
    <>
      ...
      <main className="flex-1 w-full max-w-7xl mx-auto px-6 py-12 flex flex-col justify-center items-center">
        ...
        {!user ? (
          <div className="w-full max-w-md p-8 rounded-lg panel relative overflow-hidden transition-all duration-300">
            ...
            <button 
              ...
              className="w-full flex items-center justify-center gap-3 py-3.5 px-4 rounded-lg bg-white text-gray-900 font-semibold shadow-lg hover:bg-gray-100 active:scale-[0.98] transition duration-200"
            >
            ...
            {authStatusMessage && (
              <div className="mt-6 p-3 bg-brand/10 border border-brand/30 rounded-lg text-brand text-xs text-center font-medium">
                {authStatusMessage}
              </div>
            )}

            {authError && (
              <div className="mt-6 p-3 bg-red-950/40 border border-red-800/80 rounded-lg text-red-400 text-xs text-center font-medium">
                {authError}
              </div>
            )}
          ...
          <div className="w-full space-y-10">
            ...
            <div className={`grid grid-cols-1 ${isProPayg ? 'lg:grid-cols-4' : 'lg:grid-cols-3'} gap-8`}>
              ...
              {isProPayg && (
                <div className="lg:col-span-1 p-6 rounded-lg panel border border-line relative overflow-hidden flex flex-col justify-between min-h-[220px]">
                  <div>
                    <div className="flex justify-between items-start">
                      <span className="text-xs font-bold text-ink-muted uppercase tracking-wider block">Blue Credits</span>
                      <span className="text-[10px] px-2 py-0.5 rounded font-bold bg-brand/10 border border-brand/30 text-brand">
                        PAYG
                      </span>
                    </div>
                    <span className="text-4xl font-extrabold tracking-tight mt-2 block bg-brand bg-clip-text text-transparent">
                      {Number(proWallet?.blue_credits || 0).toFixed(4)}
                    </span>
                    <span className="text-xs text-ink-faint mt-1 block">Available credits</span>
                    {hasBlueCredits && (
                      <span className="text-[20px] text-ink-faint mt-2 block font-mono">
                  ...
                  <div className="mt-6 flex space-x-3">
                    <a href="/blue-pro/checkout"
                      className="flex-1 py-2.5 px-4 rounded-lg bg-brand text-sm font-semibold text-paper shadow-md transition inline-flex items-center justify-center gap-2">
              ...
              <div className={`p-6 rounded-lg panel relative overflow-hidden flex flex-col justify-between min-h-[220px] ${isProPayg ? 'lg:col-span-1' : 'lg:col-span-1'}`}>
                <div>
                  <div className="flex justify-between items-start">
                    <span className="text-xs font-bold text-ink-muted uppercase tracking-wider block">IMR Balance</span>
                ...
                <div className="mt-6 flex space-x-3">
                  {hasBlueCredits ? null : plan === 'blue' ? (
                    ...
                    <a
                      href="/subscribe"
                      className="flex-1 py-2.5 px-4 rounded-lg bg-brand text-sm font-semibold text-paper shadow-md transition duration-200 inline-flex items-center justify-center gap-2"
                    >
              ...
              <div className="lg:col-span-2 p-6 rounded-lg panel relative overflow-hidden flex flex-col justify-between min-h-[220px]">
                {!hasBlueCredits && (
                  <div className="absolute inset-0 bg-paper/90 z-10 flex flex-col items-center justify-center p-6 text-center select-none">
                    ...
                    <h4 className="text-sm font-bold text-ink tracking-tight">Add Blue Credits to continue</h4>
                    ...
                    <a href="/blue-pro/checkout?pack=starter" className="mt-4 inline-flex items-center gap-2 rounded-lg bg-brand px-4 py-2 text-xs font-bold text-paper">
                      <i className="fa-solid fa-cart-plus"></i>
                      Buy ₹100 Trial
                    </a>
                  </div>
          ...
          {isProPayg && proWallet && (
            <div id="blue-pro-section" className="scroll-mt-24 pt-10 border-t border-line">
              <div className="flex items-center justify-between mb-6">
                <div>
                  <h3 className="text-xl font-bold tracking-tight text-ink">
                    <i className="fa-solid fa-bolt text-brand mr-2"></i>
                    Blue Pro
                  </h3>
                  <p className="text-xs text-ink-faint mt-1">Credit usage and purchase history</p>
                </div>
                <a href="/blue-pro/checkout"
                  className="px-5 py-2 rounded-lg bg-brand font-bold text-paper shadow-lg shadow-sm transition text-sm">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                <div className="p-4 rounded-lg panel border border-line">
                  <span className="text-[10px] font-bold text-ink-muted uppercase tracking-wider">Blue Credits</span>
                  <p className="text-2xl font-extrabold text-ink mt-1">{Number(proWallet.blue_credits || 0).toFixed(4)}</p>
                  <p className="text-[10px] text-ink-faint mt-0.5">Available balance</p>
                </div>
                <div className="p-4 rounded-lg panel border border-line">
                  <span className="text-[10px] font-bold text-ink-muted uppercase tracking-wider">Total Purchased</span>
                  <p className="text-2xl font-extrabold text-ink mt-1">{Number(proWallet.total_purchased || 0).toFixed(2)}</p>
                  <p className="text-[10px] text-ink-faint mt-0.5">All time</p>
                </div>
                <div className="p-4 rounded-lg panel border border-line">
                  <span className="text-[10px] font-bold text-ink-muted uppercase tracking-wider">30-Day Usage</span>
                  <p className="text-2xl font-extrabold text-ink mt-1">{proUsage?.summary?.total_blue_credits_used?.toFixed(4) || "0.0000"}</p>
                  <p className="text-[10px] text-ink-faint mt-0.5">{proUsage?.summary?.total_requests || 0} requests</p>
                </div>
              </div>

              <div className="flex gap-1.5 p-1 bg-paper border border-line rounded-lg mb-6 max-w-sm">
              ...
              {proTab === "overview" && (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  <div className="p-5 rounded-lg panel border border-line">
                    <h4 className="text-xs font-bold text-ink-muted mb-3">Recent Purchases</h4>
                    {proTransactions.filter((t: any) => t.status === "completed" || t.status === "failed").length === 0 ? (
                      <p className="text-xs text-ink-faint">No completed purchases yet.</p>
                    ) : (
                      <div className="space-y-2">
                        {proTransactions.filter((t: any) => t.status === "completed" || t.status === "failed").slice(0, 5).map((txn: any) => (
                          <div key={txn.id} className="flex items-center justify-between py-1.5 border-b border-line last:border-0">
                            <div>
                              <p className="text-xs font-semibold text-ink">{txn.credits_purchased} Blue Credits</p>
                              <p className="text-[10px] text-ink-faint">{new Date(txn.created_at).toLocaleDateString()}</p>
                            </div>
                  ...
                  <div className="p-5 rounded-lg panel border border-line">
                    <h4 className="text-xs font-bold text-ink-muted mb-3">Usage by Model (30 days)</h4>
                    {!proUsage?.summary?.model_breakdown || Object.keys(proUsage.summary.model_breakdown).length === 0 ? (
                      <p className="text-xs text-ink-faint">No usage recorded yet.</p>
                    ) : (
                      <div className="space-y-2">
                        {Object.entries(proUsage.summary.model_breakdown).map(([model, data]: [string, any]) => (
                          <div key={model} className="flex items-center justify-between py-1.5 border-b border-line last:border-0">
                            <div>
                    ...
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="border-b border-line">
                          <th className="text-left py-3 px-5 text-ink-muted font-semibold text-[10px] uppercase">Date</th>
                          <th className="text-right py-3 px-5 text-ink-muted font-semibold text-[10px] uppercase">Amount</th>
                          <th className="text-right py-3 px-5 text-ink-muted font-semibold text-[10px] uppercase">Credits</th>
                          <th className="text-right py-3 px-5 text-ink-muted font-semibold text-[10px] uppercase">Status</th>
                        </tr>
                      </thead>
                      <tbody>
                        {proTransactions.filter((t: any) => t.status === "completed" || t.status === "failed").length === 0 ? (
                          <tr><td colSpan={4} className="py-8 text-center text-ink-faint text-xs">No purchases yet.</td></tr>
                        ) : proTransactions.filter((t: any) => t.status === "completed" || t.status === "failed").map((txn: any) => (
                          <tr key={txn.id} className="border-b border-line hover:bg-paper-alt transition">
                            <td className="py-3 px-5 text-ink-muted text-xs">{new Date(txn.created_at).toLocaleString()}</td>
                            <td className="py-3 px-5 text-right text-ink-muted text-xs">${Number(txn.amount_paid).toFixed(2)}</td>
                            <td className="py-3 px-5 text-right text-brand font-mono text-xs">{Number(txn.credits_purchased).toFixed(2)}</td>
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="border-b border-line">
                          <th className="text-left py-3 px-5 text-ink-muted font-semibold text-[10px] uppercase">Date</th>
                          <th className="text-left py-3 px-5 text-ink-muted font-semibold text-[10px] uppercase">Model</th>
                          <th className="text-right py-3 px-5 text-ink-muted font-semibold text-[10px] uppercase">Input</th>
                          <th className="text-right py-3 px-5 text-ink-muted font-semibold text-[10px] uppercase">Output</th>
                          <th className="text-right py-3 px-5 text-ink-muted font-semibold text-[10px] uppercase">Cost</th>
                        </tr>
                      </thead>
                      <tbody>
                        {!proUsage?.usage || proUsage.usage.length === 0 ? (
                          <tr><td colSpan={5} className="py-8 text-center text-ink-faint text-xs">No usage recorded yet.</td></tr>
                        ) : proUsage.usage.map((row: any, i: number) => (
                          <tr key={i} className="border-b border-line hover:bg-paper-alt transition">
                            <td className="py-3 px-5 text-ink-muted text-[10px]">{new Date(row.created_at).toLocaleString()}</td>
            ...
            <div>
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
                <div>
                  ...
                  <p className="text-xs text-ink-faint mt-1">Blue Pro models organized by credit cost tier — from everyday coding to maximum intelligence.</p>
                </div>
              ...
              <div className="flex flex-col md:flex-row gap-4 justify-between items-stretch md:items-center mb-6">
                <div className="flex flex-wrap gap-1.5 p-1 bg-paper border border-line rounded-lg max-w-full overflow-x-auto">
                  {[
                ...
                <div className="relative md:w-64">
                  <span className="absolute inset-y-0 left-0 pl-3 flex items-center text-ink-faint">
                    <i className="fa-solid fa-magnifying-panel text-xs"></i>
                  </span>
                  <input
                    type="text"
                    value={searchQuery}
                    ...
                    className="w-full bg-paper-alt border border-line rounded-lg py-2 pl-9 pr-4 text-xs text-ink focus:outline-none focus:border-brand focus:ring-1 focus:ring-brand/20 transition"
                  />
                  {searchQuery && (
                    <button
              ...
              {(() => {
                if (modelsLoading) {
                  return (
                    <div className="text-center py-12 rounded-lg panel border border-line p-8">
                      <div className="w-8 h-8 rounded-full border-2 border-line border-t-blue-500 animate-spin mx-auto mb-3"></div>
                      <h4 className="text-sm font-semibold text-ink-muted">Loading the model catalog</h4>
                      <p className="text-xs text-ink-faint mt-1">Fetching the latest models and pricing from the provider.</p>
                    </div>
                  );
                }
                ...
                if (modelsError) {
                  return (
                    <div className="text-center py-12 rounded-lg panel border border-red-900/60 p-8">
                      ...
                      <h4 className="text-sm font-semibold text-ink">The model catalog is temporarily unavailable</h4>
                      <p className="text-xs text-ink-faint mt-1">{modelsError}</p>
                      <button
                ...
                if (filteredModels.length === 0) {
                  return (
                    <div className="text-center py-12 rounded-lg panel border border-line p-8">
                      <div className="w-12 h-12 rounded-full bg-paper-alt border border-line flex items-center justify-center mx-auto mb-3">
                        <i className="fa-solid fa-magnifying-panel text-ink-faint"></i>
                      </div>
                      <h4 className="text-sm font-semibold text-ink-muted">No models match your search</h4>
                      <p className="text-xs text-ink-faint mt-1">Try clearing your search query or selecting a different tier tab.</p>
                      <button
                        onClick={() => {
                          setSearchQuery("");
                          setSelectedCategory("all");
                        }}
                        className="mt-4 px-4 py-1.5 rounded-lg border border-line text-xs font-semibold text-ink-muted hover:text-ink hover:bg-paper-sunken transition"
                      >
                        Reset Filters
                      </button>
                    </div>
                ...
                return (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-h-[60vh] overflow-y-auto pr-2 custom-scrollbar">
                    {filteredModels.map((model) => (
                      <div
                        key={model.id}
                        className="p-6 rounded-lg panel border border-line hover:border-line-strong hover:bg-paper-alt transition duration-300 flex flex-col justify-between"
                      >
                        <div>
                          <div className="flex justify-between items-start gap-4 mb-3">
                            <div>
                              <span className="text-md font-bold tracking-tight block text-ink">
                                {model.displayName}
                              </span>
                            ...
                            <span className={`text-[10px] px-2 py-0.5 rounded font-bold ${TIER_BADGE_COLORS[model.tier] || "bg-paper/60 text-ink-muted border border-line"}`}>
                              {model.tier}
                            </span>
                          </div>
                          <p className="text-xs text-ink-muted leading-relaxed min-h-[40px]">
                            {model.description}
                          </p>
                        </div>
                        <div className="mt-6 pt-4 border-t border-line flex justify-between items-center text-xs">
                          <span className="text-ink-faint">
                            Input: <strong className="text-ink-muted font-mono">{model.inputCredits} credits/1M</strong> • Output: <strong className="text-ink-muted font-mono">{model.outputCredits} credits/1M</strong>
                          </span>
      ...
      <footer className="w-full panel py-6 px-6 border-t border-line text-center text-xs text-ink-faint">
          ...
          <div className="w-full max-w-md rounded-lg panel border border-line p-6 relative overflow-hidden transition-all duration-300 transform scale-100">
            ...
            <div className="flex items-start gap-4">
              <div className={`w-10 h-10 rounded-lg flex items-center justify-center shrink-0 shadow-lg ${
                modalConfig.type === 'success'
                  ? 'bg-green-950/40 border border-green-800/50 text-green-400 shadow-sm'
                  : modalConfig.type === 'error'
              ...
              <div className="flex-1 min-w-0">
                <h3 className="text-lg font-bold text-ink tracking-tight leading-snug">
                  {modalConfig.title}
                </h3>
                <p className="mt-2 text-sm text-ink-muted leading-relaxed font-medium">
                  {modalConfig.message}
                </p>
              </div>
            </div>
              ...
              {modalConfig.type === 'confirm' ? (
                <>
                  <button
                    ...
                    className="px-4 py-2 rounded-lg border border-line text-sm font-semibold text-ink-muted hover:text-ink hover:bg-paper-sunken transition duration-200"
                  >
                    Cancel
                  </button>
                  <button
                    onClick={modalConfig.onConfirm}
                    className="px-5 py-2 rounded-lg bg-brand text-sm font-semibold text-paper shadow-lg active:scale-[0.98] transition duration-200"
                  >
                    Confirm
                  </button>
                </>
                ...
                <button
                  ...
                  className="px-6 py-2 rounded-lg bg-brand text-sm font-semibold text-paper shadow-lg active:scale-[0.98] transition duration-200"
                >
                  OK
                </button>
              )}
