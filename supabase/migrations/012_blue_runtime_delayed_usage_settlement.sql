-- Keep provider credentials disabled but queryable until delayed authoritative
-- usage has stabilized. Reservations stay locked while a task is settling.

alter table public.blue_runtime_tasks
  add column if not exists client_provider_cost_hint numeric(18, 10) not null default 0
    check (client_provider_cost_hint >= 0),
...
alter table public.blue_runtime_credentials
  add column if not exists usage_observed_at timestamptz,
  add column if not exists usage_stable_observations integer not null default 0
    check (usage_stable_observations >= 0);

create index if not exists blue_runtime_tasks_settlement_retry_idx
  on public.blue_runtime_tasks (settlement_next_attempt_at, updated_at)
  where state = 'stopping';

-- A task whose provider credential is already disabled is no longer executing.
-- Do not let delayed billing reconciliation consume a trial/PAYG execution slot.
